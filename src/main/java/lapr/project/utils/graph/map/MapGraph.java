...
// Constructs an empty graph (either undirected or directed)
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        ...
        for(Map.Entry<V, MapVertex<V,E>> geralMap : mapVertices.entrySet()){
            for(Edge<V,E> subMap : geralMap.getValue().getAllOutEdges()){
                if(subMap.getVDest().equals(vert)){
                    incomingEdges.add(subMap);
                }
            }
        }
